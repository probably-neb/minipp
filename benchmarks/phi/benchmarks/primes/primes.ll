...
while.cond11:
  %_12 = icmp sle i64 %square9, %a
  br i1 %_12, label %while.body2, label %while.exit5
while.body2:
  %a1 = phi i64 [ %a, %while.cond11 ], [ %a1, %while.fillback4 ]
  %square3 = phi i64 [ %square9, %while.cond11 ], [ %square14, %while.fillback4 ]
  %delta5 = phi i64 [ %delta10, %while.cond11 ], [ %delta16, %while.fillback4 ]
  %square14 = add i64 %square3, %delta5
  %imm_store15 = add i64 2, 0
  %delta16 = add i64 %delta5, %imm_store15
...
while.exit5:
  %a2 = phi i64 [ %a, %while.cond11 ], [ %a1, %while.cond23 ]
  %square4 = phi i64 [ %square9, %while.cond11 ], [ %square14, %while.cond23 ]
  %delta6 = phi i64 [ %delta10, %while.cond11 ], [ %delta16, %while.cond23 ]
  %imm_store21 = add i64 2, 0
  %tmp.binop22 = sdiv i64 %delta6, %imm_store21
  %imm_store23 = add i64 1, 0
  %tmp.binop24 = sub i64 %tmp.binop22, %imm_store23
  br label %exit
exit:
  %return_reg25 = phi i64 [ %tmp.binop24, %while.exit5 ]
...
body0:
  br label %if.cond1
if.cond1:
  %imm_store16 = add i64 2, 0
  %_17 = icmp slt i64 %a, %imm_store16
  br i1 %_17, label %then.body2, label %else.body3
then.body2:
  %imm_false19 = or i1 0, 0
  br label %exit
else.body3:
  %max22 = call i64 (i64) @isqrt(i64 %a)
  %divisor23 = add i64 2, 0
  br label %while.cond14
while.cond14:
  %_25 = icmp sle i64 %divisor23, %max22
...
while.body5:
  ...
  %divisor8 = phi i64 [ %divisor23, %while.cond14 ], [ %divisor37, %while.fillback10 ]
  %remainder10 = phi i64 [ %remainder45, %while.cond14 ], [ %remainder29, %while.fillback10 ]
...
if.cond6:
  ...
  %_32 = icmp eq i64 %remainder29, %imm_store31
  br i1 %_32, label %then.body7, label %if.exit8
then.body7:
  %imm_false34 = or i1 0, 0
  br label %exit
if.exit8:
  %imm_store36 = add i64 1, 0
  %divisor37 = add i64 %divisor8, %imm_store36
...
while.exit11:
  %max3 = phi i64 [ %max22, %while.cond14 ], [ %max2, %while.cond29 ]
  %a6 = phi i64 [ %a, %while.cond14 ], [ %a4, %while.cond29 ]
  %divisor9 = phi i64 [ %divisor23, %while.cond14 ], [ %divisor37, %while.cond29 ]
  %remainder12 = phi i64 [ %remainder45, %while.cond14 ], [ %remainder29, %while.cond29 ]
  %imm_true42 = or i1 1, 0
  br label %exit
exit:
  %return_reg20 = phi i1 [ %imm_false19, %then.body2 ], [ %imm_false34, %then.body7 ], [ %imm_true42, %while.exit11 ]
  ret i1 %return_reg20
}
...
while.fillback8:
  br label %while.body2
while.exit9:
  %limit1 = phi i64 [ %limit9, %while.cond11 ], [ %limit0, %while.cond27 ]
  %a3 = phi i64 [ %a10, %while.cond11 ], [ %a22, %while.cond27 ]
  %imm_store27 = add i64 0, 0
  br label %exit
exit:
  %return_reg28 = phi i64 [ %imm_store27, %while.exit9 ]
